...
void help(){
    std::cout<<"Usage:"<<std::endl<<"    so_projekt1.exe <dataType> <size> <sorting> <outputFile> <numbersFile> <pivot>"<<std::endl;
...
int main(const int argc, char* argv[]) {
    if (argc == 7) {
        ...
        Benchmark benchmark(dataType,size,sorting,outputFile,numbersFile,pivot);
        benchmark.start();
